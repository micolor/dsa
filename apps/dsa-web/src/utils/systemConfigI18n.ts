...
const fieldTitleMap: Record<string, string> = {
  ...
  NOTIFICATION_EVENT_CHANNELS: '事件通知渠道',
  ...
  AGENT_SKILL_CONCURRENCY: '策略技能并发数',
...
const fieldDescriptionMap: Record<string, string> = {
  ...
  ANTHROPIC_API_KEY: 'Anthropic Claude API 密钥（从 console.anthropic.com 获取）。',
  ANTHROPIC_API_KEYS: '逗号分隔的多个 Anthropic API 密钥，用于负载均衡；优先级高于 ANTHROPIC_API_KEY。',
  ANTHROPIC_MODEL: 'Claude 模型名称（如 claude-sonnet-4-6）。',
  ANTHROPIC_TEMPERATURE: '温度参数，范围 [0.0, 1.0]。',
  ANTHROPIC_MAX_TOKENS: 'Anthropic API 响应最大 token 数（默认 8192）。',
  ...
  DEEPSEEK_API_KEYS: '逗号分隔的多个 DeepSeek API 密钥，用于负载均衡；优先级高于 DEEPSEEK_API_KEY。',
  GEMINI_API_KEY: '用于 Gemini 服务调用的密钥。',
  GEMINI_API_KEYS: '逗号分隔的多个 Gemini API 密钥，用于负载均衡；优先级高于 GEMINI_API_KEY。',
  GEMINI_MODEL: '设置 Gemini 分析模型名称。',
  GEMINI_TEMPERATURE: '控制模型输出随机性，范围通常为 0.0 到 2.0。',
  GEMINI_MODEL_FALLBACK: 'Gemini 备选模型名称；未配置 LITELLM_FALLBACK_MODELS 且主模型为 Gemini 时使用。',
  OPENAI_API_KEY: '用于 OpenAI 兼容服务调用的密钥。',
  OPENAI_API_KEYS: '逗号分隔的多个 OpenAI 兼容 API 密钥，用于负载均衡；优先级高于 AIHUBMIX_KEY 与 OPENAI_API_KEY。',
  OPENAI_BASE_URL: 'OpenAI 兼容 API 地址，例如 https://api.deepseek.com/v1。',
  OPENAI_VISION_MODEL: '图片识别所用模型（部分 API 如 DeepSeek 无视觉能力）；留空则使用 OPENAI_MODEL。',
  OPENAI_TEMPERATURE: 'OpenAI 兼容模型的温度参数，范围 [0.0, 2.0]。',
  ...
  FEISHU_CHAT_ID: '飞书应用机器人的推送目标：群聊模式填 chat_id（oc_ 开头），私聊模式填 open_id（ou_ 开头）；需同时配置 FEISHU_APP_ID 与 FEISHU_APP_SECRET。',
  FEISHU_RECEIVE_ID_TYPE: 'FEISHU_CHAT_ID 的取值类型：chat_id 为群聊，open_id 为私聊。',
  FEISHU_DOMAIN: '飞书 API 域名：feishu 为国内版（feishu.cn），lark 为国际版（larksuite.com）。',
  ...
  NOTIFICATION_EVENT_CHANNELS: '事件型通知（龙虎榜、主力资金、重要公告）的发送渠道，多个值用英文逗号分隔；留空表示沿用所有已配置渠道。',
  ...
  PAPER_NOTIFY_ENABLED: '模拟盘产生实时成交（开仓、加仓、减仓、清仓与止损 / 止盈平仓）时推送到通知渠道；走 event 路由，需先配好对应渠道。历史回填是重放，全程不推送。',
  ...
  AGENT_SKILL_CONCURRENCY: "专家模式下同时运行的策略技能 Agent 数量上限。",
