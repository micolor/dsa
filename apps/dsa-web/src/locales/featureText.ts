...
export const ALERT_PAGE_TEXT = {
  zh: { documentTitle: '告警中心 - DSA', eyebrow: '告警中心', tabsRules: '告警规则', tabsHistory: '触发历史', tabsNotifications: '通知尝试记录', createSuccess: '创建成功', createRule: '新建告警规则', editRule: '编辑告警规则', created: '已创建告警规则「{name}」', updated: '已更新告警规则「{name}」', close: '关闭', testResult: '测试结果', testYes: '是', testNo: '否', testTriggered: '触发：{value}', testObserved: '观察值：{value}', testEvaluated: '共评估 {count} 个目标：触发 {triggered} · 降级 {degraded} · 跳过 {skipped}', testSkippedHint: '有 {count} 个目标因缺少实时行情数据被跳过，未能参与判断——这不代表「未触发」，常见于非交易时段、停牌或实时报价缺失时。', notificationEyebrow: '通知结果', notificationTitle: '通知尝试记录', notificationLoading: '正在加载通知尝试记录', notificationEmptyTitle: '暂无通知尝试记录', notificationEmptyDescription: '当前没有可展示的通知尝试明细；告警触发仍会按已配置通知渠道发送。', channelCol: '渠道', statusCol: '状态', errorCodeCol: '错误码', latencyCol: '耗时', timeCol: '时间', diagnosticsCol: '诊断', dash: '--' },
  en: { documentTitle: 'Alert Center - DSA', eyebrow: 'Alert center', tabsRules: 'Rules', tabsHistory: 'Trigger history', tabsNotifications: 'Notification attempts', createSuccess: 'Created', createRule: 'New alert rule', editRule: 'Edit alert rule', created: 'Created alert rule "{name}"', updated: 'Updated alert rule "{name}"', close: 'Close', testResult: 'Test result', testYes: 'Yes', testNo: 'No', testTriggered: 'Triggered: {value}', testObserved: 'Observed: {value}', testEvaluated: 'Evaluated {count} targets: triggered {triggered} · degraded {degraded} · skipped {skipped}', testSkippedHint: '{count} targets were skipped for lack of real-time quotes and did not take part in the judgment — this does not mean "not triggered"; it is common off-market-hours, during suspension, or when real-time quotes are missing.', notificationEyebrow: 'Notification results', notificationTitle: 'Notification attempts', notificationLoading: 'Loading notification attempts', notificationEmptyTitle: 'No notification attempts', notificationEmptyDescription: 'No notification attempt details to show; triggered alerts still send through configured channels.', channelCol: 'Channel', statusCol: 'Status', errorCodeCol: 'Error code', latencyCol: 'Latency', timeCol: 'Time', diagnosticsCol: 'Diagnostics', dash: '--' },
} as const;
