...
>
> **P2. 给 chip 的标签与图片相关的用户可见提示补 i18n。** Task 5 里它们是硬编码中文，与相邻 composer 的既有风格一致，但在英文界面下是**可见**缺口。两类都要补（zh + en 两段）：
> - chip 的三个标签：`待发送的图片` / `移除图片` / `添加图片`；
> - **五条图片提示**：`不支持的图片类型…`、`图片过大（上限 …MB）…`（这条的 MB 数由常量派生，见 Task 5 的实现）、`部分图片未加入…`、`图片已替换…`。
>
> 测试环境是中文（无 `UiLanguageProvider` 时回退 zh），所以断言里的中文字面量不必改——但要确认确实如此再动手。
>
