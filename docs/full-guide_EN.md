...
- **Image paste in Ask Stock** - The Ask Stock composer accepts a pasted (Ctrl+V), dropped, or picked image (jpg/png/webp/gif, up to 2MB); the composer shows "Reading image…" while the vision model reads it, the answer is based on that image, any stock codes it contains are listed, and an explicit request can add them to the watchlist through the existing confirmation card. The image itself is never saved, so it cannot be reopened after a refresh (the session title and the message bubble keep only what you typed).
