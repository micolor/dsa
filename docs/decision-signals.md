...
- 「AI 建议」页首屏为「待处理」区，启用中的信号按「可以动手 / 需要确认 / 观察」三档分组。分档规则是两级短路判定（`apps/dsa-web/src/utils/decisionSignalQueue.ts` 的 `classifyDecisionSignalQueue`）：**先**看 `action` 是否属于会改仓位的集合 `{buy, add, reduce, sell}`，**再**看 `plan_quality` 是否恰好等于 `complete`。因此只有 `action ∈ {buy, add, reduce, sell}` 且 `plan_quality = complete` 进入「可以动手」；同一改仓位动作但 `plan_quality` 不是 `complete`（`partial`、`minimal`、`unknown`，以及后端不会下发的越界取值）一律进入「需要确认」；`action` 落在改仓位集合之外时（`hold`、`watch`、`avoid`、`alert`，以及未知或缺失取值）无论 `plan_quality` 为何都进入「观察」，且「观察」档默认收起。`plan_quality` 只对改仓位的动作起作用，三档覆盖全部输入，不存在兜底桶。
- 分档只看 `action` 与 `plan_quality` 两个字段：`stop_loss` 不参与判定，因此 `plan_quality = complete` 并不保证信号带有止损位，「可以动手」档内完全可能出现没有止损的计划；`needs_review` 与 `watch` 都不是可执行档，未知取值落入哪一档都不会把「不该动手」误报成「可以动手」。
- 「非小写 / 越界 `action` 不可达」的保证来自**写入路径**，不是存储或响应类型：创建请求 `DecisionSignalCreateRequest` 的 `action` 与 `plan_quality` 分别声明为 `DecisionAction` 和 `DecisionSignalPlanQuality` 这类 `Literal`，非法或大写取值在进入服务层之前就被 Pydantic 以 HTTP 422 拒绝；服务层另有 `_normalize_action`（统一小写并拒绝非法取值）和 `_normalize_enum`（只接受精确匹配，失败抛 `ValueError`，由 endpoint 转为 HTTP 400 `validation_error`）。所以经公开 API 写入的信号不会出现非小写或越界的 `action` / `plan_quality`。但 `src/storage.py` 的 `decision_signals.action` 是普通 `String(16)` 列，列表响应的 `DecisionSignalItem.action` 也声明为 `str` 而不是 `Literal`；历史脏数据或绕过 API 的直写仍可能带来越界取值，Web 分档因此对越界值给出明确定义，而不是假定它不会出现。
- 「待处理」区分档基于单独一次 `status=active`、`page_size=100` 的列表请求，不受主列表分页与主列表筛选影响；结果达到 100 条时 Web 显示「仅在最近 100 条启用中的信号内分档。」，不静默截断。仅当列表没有任何活跃筛选（与 `DEFAULT_LIST_FILTERS` 逐字段比较，含 `status` 与 `sourceReportId`）且开启「去重最新」时，主列表自身即以 `status=active`、`page_size=100` 且不分页的方式拉取，与待处理区这次宽查询等价，此时「待处理」区复用主列表结果、不重复发请求（`DEDUPE_PAGE_SIZE` 与 `QUEUE_PAGE_SIZE` 同为 100，复用成立依赖这一点）。只要存在任何活跃筛选（例如把状态改回「全部状态」得到 `status=''`，或经 `?sourceReportId=` deep link 进入），主列表就会带上这些筛选而待处理区不会，两者不再等价，「待处理」区必须照常发自己的宽查询——否则「可以动手」会展示被筛掉的、甚至已失效的信号，或漏掉筛选项之外的可执行信号。「只看我的持仓」不是列表筛选（不在 `appliedFilters` 里），它同时作用于两次请求，不影响上面这条判断。
- 「只看我的持仓」开关复用列表 API 既有 `holding_only` 查询参数，同时作用于「待处理」区与主列表（`apps/dsa-web/src/api/decisionSignals.ts` 早已透出该参数，此前页面从未使用）；后端未新增参数或接口。
- 主列表筛选表单默认收起；当筛选条件与 `DEFAULT_LIST_FILTERS` 逐字段相异时自动展开（默认的 `status=active` 不计为活跃筛选），自动展开只在活跃筛选出现时发生一次，之后用户可自由收起。既有 `activeFilterChips` 恒非空的行为未改动。
- 「信号表现统计」与「Skill 表现」两张卡位于页面底部默认收起的「统计数据」区。**这是一项行为变更**：此前两张卡在页面挂载时即各自发起 `getOutcomeStats` 与 `getSkillOutcomeStats` 请求，现在只在展开该区时才请求，折叠状态下卡片内容不挂载，统计区自身在首屏发出的请求由两次降为 0 次（页面另有「待处理」区的列表请求，两者不是同一笔）；「刷新」按钮语义不变，只在该区已展开时刷新统计。
- Skill 表现表的 skill 列显示当前语言的 skill 名称（如 `box_oscillation` 显示为「箱体震荡」）。skill 是开放集合（`AGENT_SKILLS` 支持配置自定义策略，如 `mean_reversion`），未知 skill id 回退显示原始 id，不回退为 `-` 占位——`getSkillLabel` 在这一点上与同文件其他标签函数是有意不同的。内建策略的映射与后端 `src/report_language.py` 的 `_STRATEGY_SKILL_TRANSLATIONS` 的一致性由 `tests/test_skill_label_parity.py` 守护：该测试要求两边 id 集合严格相等，并逐条比对 zh / en 文案本身（只查 key 存在性无法发现改字）。
