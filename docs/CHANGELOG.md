...
## [Unreleased]
- [修复] 选股页恢复本地结果副本时不做任何形状校验，一个字段缺失的候选就会把整页打成「页面加载失败」：副本是 `readScreenResult` 里原样 `JSON.parse` 回来的（`as ScreeningScreenResponse` 只是编译期断言，运行时没有任何保证），而渲染链路默认每个候选都是完整对象——`candidateFormat.getSignal` 直接读 `item.raw.action`、结果区直接 `candidates.map(...)`（`CandidateListItem` 内部对其它列表字段用的是 `?.length` 之类的可选链，这两处是漏的）。浏览器实测：写入 `dsa.screening.lastResult.v1` = `{strategy:'dual_low', market:'cn', candidates:[{code:'600519', name:'贵州茅台', score:88}]}`（即缺 `raw` 的候选）后刷新，`CandidateDetail`（首个候选默认展开）抛 `TypeError: Cannot read properties of undefined (reading 'action')`，`RouteErrorBoundary` 把整个 `/screening` 换成「页面加载失败 / 当前页面资源或组件未能正常加载」；把副本的 `candidates` 写成非数组（如字符串）则是 `TypeError: candidates.map is not a function`（`StockScreeningPage.tsx` 的 `result.candidates || []`：真值非数组会穿过兜底）。现按「不可信输入在入口收口」修完整契约而非单点：新增 `toScreenCandidates`（非数组按空列表处理、数组里的非对象元素丢掉），`readScreenResult` 对 `candidates` 不是数组的副本整块丢弃（与其既有的「副本不属于当前表单就丢弃」策略一致，避免留下后按「0 条候选」渲染出一次从未发生的空结果），`applyScreenResult` 走同一收敛函数覆盖服务端响应与任务轮询两条活数据路径，`getSignal` 对缺失/非对象的 `raw` 退化为默认信号「观察」。新增回归 2 例（缺 `raw` 的副本必须正常渲染且展开区显示「观察」；`candidates` 非数组的副本必须整块丢弃、结果区不出现），反向验证：修复前两例分别以 `reading 'action'` 与 `candidates.map is not a function` 转红，浏览器里两个载荷也各自复现崩溃、修复后分别正常渲染。注：后端不产出这种载荷——`_normalize_candidate` 恒写 `"raw": source`（`source` 必为 dict），且 `/screen` 端点无 `response_model` 过滤，副本又是原样存整份响应；因此触发条件是外部改写的 localStorage / 旧版本残留 / 写了一半的副本，不是正常后端数据，修复属加固与契约收敛

- [修复] 容器健康检查在服务模式下永远报 healthy：`docker/Dockerfile` 的 `HEALTHCHECK` 末尾是 `|| python -c "import sys; sys.exit(0)"`，第三段无条件成功，于是两条 curl 打不通时退出码仍是 0——服务挂死、API 不可达、甚至进程还在但 uvicorn 事件循环被阻塞，`docker ps` / compose / 编排平台一律显示 `healthy`，而文档把「看健康检查」写成了第一线排障手段，等于在对的时候给出相反的结论。端口也写死 8000，与文档里 `API_PORT=8888`（宿主机端口冲突）和 `API_PORT=8080` 的用法不符：这些部署两条 curl 都打空，直接落到无条件成功那段。现改为 `docker/healthcheck.sh`（新增，`HEALTHCHECK CMD` 以 exec 形式调用）：读容器内 PID 1 的实际启动命令（`entrypoint.sh` 末尾是 `exec "$@"`）判断本次是否起服务，只有服务模式（`--serve` / `--serve-only` / `--webui` / `--webui-only`，或 `WEBUI_ENABLED=true`）才探测 HTTP，端口按 `--port` → `API_PORT` → 8000 的顺序取实际绑定值，探测失败即 `exit 1`；定时任务容器（`python main.py --schedule`，无 HTTP 端点）仍按既有约定始终健康，不会因为「本来就没有服务」被判 unhealthy。新增回归 8 例（真实起本地 HTTP 服务：服务模式探测失败必须非 0、探测成功必须为 0、端口跟随 `--port` 与 `API_PORT`、定时任务模式与读不到 cmdline 时不得虚构 unhealthy、Dockerfile 接线与无条件兜底不得回归），反向验证：把末尾改回无条件成功则 3 例转红。注：本机 Docker daemon 未运行，未做真实 `docker build`/`HEALTHCHECK` 运行，验证的是脚本本身与 Dockerfile 接线

- [修复] `FEISHU_SEND_AS_FILE=true` 时，`route_type="report"` 的通用飞书发送会把当天的规范日报文件覆盖成别的内容：`NotificationService._send_to_static_channel` 的 FEISHU 分支调用 `save_report_to_file(sanitized_content, filename=f"report_{date}.md")`，而该文件名正是 `pipeline._save_local_report` 写入当天完整决策仪表盘所用的路径（`reports/report_{YYYYMMDD}.md`），`save_report_to_file` 又是 `open(..., 'w')` 截断写入；同时 `generate_wechat_dashboard` 会把 `reports/report_{YYYYMMDD}.md` 作为「详细报告见 …」的承诺路径写进企业微信正文。实测：先按 `_save_local_report` 的口径写入一份「# 决策仪表盘 …」全文，再以 `route_type="report"` 发送 `复盘文档创建成功: https://feishu.cn/docx/XXXX`，该文件被替换成这一行链接（同一份被覆盖的内容随后作为 `.md` 附件发给飞书），当天本地产物被不可逆销毁，用户按企业微信提示取到的是无关内容。真实日更时序里这个推送排在保存日报之后（`main.py` 里飞书文档创建成功后那条推送、以及 `--market-review` 的复用路径），因此必现。现改为用一个临时目录里的 `report_{date}.md` 承载附件、上传后即删，不再触碰 `reports/` 下的任何规范路径（这与 `pipeline` 自己的飞书分支改用 `dashboard_{date}.md`、`main.py` 保存大盘复盘改用 `market_review_{date}.md` 的既有区分一致）。更新测试 2 例并新增回归 1 例（日报在附件投递后必须字节不变；附件路径必须落在临时目录且正文仍做隐藏元数据剥离），反向验证：还原旧调用点 3 例全红

- [修复] 自动打 tag 把「正文里提到 `#patch`」也当成发版请求：`.github/workflows/auto-tag.yml` 的 job 级 `if:` 用的是 `contains(github.event.head_commit.message, '#patch' | '#minor' | '#major')`，而 `head_commit.message` 是**整条** commit message，squash 合并会把被合并分支的提交正文（包括别人写的裸 `#patch` 行）一并带进来，于是内容里出现这几个字就推一个没人要求的版本号——与 AGENTS.md §7「只有 commit title 含 `#patch`/`#minor`/`#major` 才触发版本号更新」直接冲突。用真实历史复现：全量扫描 `git log` 的 title 与正文后，20 条带标记的提交里 10 条标记在 title（应发版），另 **10 条标记只在正文**，其中 7 条改动路径不在 `paths-ignore` 内、会真的走进 tag job（例：`c4e94203` title 为「fix: 修正 akshare 腾讯数据源字段索引和 ETF 行情字段映射 (#579)」，正文含一行来自被 squash 提交的裸 `#patch`，改动文件是 `data_provider/akshare_fetcher.py`、`data_provider/efinance_fetcher.py`）。现改为在 job 内新增 `Detect release marker in commit title` 步，取 message 第一行匹配标记（`${COMMIT_MESSAGE%%$'\n'*}`），检出与发版两步都挂在该步输出上，标记为空则一步都不跑；`DEFAULT_BUMP` 也从写死的 `patch` 改为 title 里读到的档位。新增回归 1 例：直接执行该步的脚本，喂入上述真实 squash 提交正文 → 输出空；title 带 `#patch`/`#minor`/`#major` → 输出对应档位；正文里更大的标记不得顶掉 title；commit message 为空（如分支删除事件）不得崩也不得发版。反向验证：还原旧的 `contains(...)` 门禁该例转红。注：本机无法访问 GitHub，未能验证 `anothrNick/github-tag-action` 自身选取档位时是否也扫正文（已把 `DEFAULT_BUMP` 对齐到 title 档位作为兜底），因此残留风险是「档位偏大」，不再是「凭空发版」

- [修复] 手动发布 Docker 镜像（Docker Manual Publish）构建的是触发分支的代码，却挂上调用方填写的版本号：`.github/workflows/ghcr-dockerhub.yml` 里唯一的 checkout 是 `actions/checkout@v5`（`fetch-depth: 0`、**没有 `ref:`**），`inputs.image_tag` 只被写进 metadata 的 tag 列表和 summary，全程没有任何一步检出该 tag。于是维护者在默认分支上点一次「重发 v3.20.0」（典型场景：基础镜像爆 CVE，要在不改代码的前提下重建镜像），推上去的其实是当时默认分支的 HEAD：镜像自称 v3.20.0，内容却是未发布的开发代码；同时两个 metadata step 都无条件带 `type=raw,value=latest`，重发旧版本会把 `latest` 盖回旧代码。现与 `docker-publish.yml` 对齐：新增 `Resolve release ref` 步骤校验 `^v[0-9]+\.[0-9]+\.[0-9]+$`、`git fetch --tags --force` 后确认 tag 存在并 `git checkout "$RELEASE_TAG"`，`latest` 改为仅当该 tag 等于最新 release tag（`sort -V` 取末尾）时才发布，并把 `DSA_WEB_VERSION`/`DSA_WEB_REVISION` 构建参数与 summary 的来源 ref 一并补上，避免 WebUI 版本卡片把开发版本误报成发布版本。新增回归 1 例（`test_manual_docker_publish_builds_the_requested_release_tag`，断言必须先解析 ref 再 smoke/构建、两个 metadata step 的 `latest` 都带 `enable=` 条件），反向验证：还原旧 workflow 该例转红。注：按 AGENTS.md §6，本地未执行 GitHub Actions，只在临时工作树里用真实 tag 列表验证了这段 shell 的 ref 解析与 `git checkout` 行为

- [修复] 关联板块的空结果被按 24h 缓存，一次失败会让该股当天的板块信息永久消失：`DataFetcherManager.get_belong_boards` 在每个源上把空结果记为失败（`error_type=empty`）后继续下一个源，但循环结束无条件执行 `boards = boards_result if boards_result is not None else []` 并写入 24h 缓存（`_BELONG_BOARDS_CACHE_TTL_SECONDS`），于是「所有源都没答上来」被当成「该股确实没有板块」。实测：一个先抛错后恢复的源，第一次调用返回 `[]`，把源改成正常后第二次调用连源都没有再问（`calls` 停在 1）仍返回 `[]`；下游 `_attach_belong_boards_to_fundamental_context` 写入空列表，通知渲染器对空板块提前返回，报告里的关联板块章节整天静默消失且没有任何降级提示。现改为只缓存非空结果（与纸面交易 bar 缓存「空结果不缓存」的既有约定一致），数据源恢复后可以立刻重新取到。新增回归 2 例（失败后恢复必须重新拉取、成功结果仍走缓存），反向验证：还原无条件写缓存则该例转红

- [修复] 港股实时行情的新浪兜底把来源标成东财：`AkshareFetcher._get_hk_realtime_quote` 在 `ak.stock_hk_spot_em()` 失败（或东财熔断）后走 `ak.stock_hk_spot()`，但构建 `UnifiedRealtimeQuote` 时仍写 `source=RealtimeSource.AKSHARE_EM`，与主分支同值，下游无法区分（熔断器 key `em_key`/`sina_key` 本身是分开的，错的只是对外暴露的来源值）。实测：让 `stock_hk_spot_em` 抛错、`stock_hk_spot` 返回腾讯控股数据，`hk00700` 得到 `source=RealtimeSource.AKSHARE_EM`；影响面是来源展示（`enhanced['realtime']['source']`）、数据质量告警的 `_source_label`（`base.py`）与 provider-run 记录，排查港股异常行情时会被指向错误的上游。现改为 `RealtimeSource.AKSHARE_SINA`。新增回归 1 例，反向验证：改回 `AKSHARE_EM` 该例转红

- [修复] YfinanceFetcher 把沪市 605 / 689 / 900 段 A 股误判为深市，兜底源因此请求不存在的 Yahoo 代码：`_convert_stock_code` 的交易所表只认 `600/601/603/688`（沪）与 `000/002/300`（深），`605`（沪市主板，如 605499 东鹏饮料）、`689`（科创板 CDR）、`900`（沪市 B 股）落在末尾的「无法确定股票 X 的市场，默认使用深市」分支，被拼成 Yahoo 上不存在的 `605499.SZ`；实测 `605499/689009/900901` 均返回 `.SZ` 并附带该误导性告警，而 `tushare_fetcher` / `baostock_fetcher` 的同名表都包含 `605`，说明漏的是这一处。YfinanceFetcher 在 A 股日线链路里是 P4 的兜底成员，命中时 Yahoo 无数据、`_fetch_raw_data` 抛错并白耗一次尝试（还要赔上 tenacity 的 3 次重试），使用者若把腾讯移出优先级链则会直接失败。现把前缀表补齐为沪市 `600/601/603/605/688/689/900`、深市 `000/001/002/003/200/300/301`（与同链路适配器一致），并让未知代码段直接抛 `ValueError` 而不是猜一个后缀——猜出来的后缀只会换来注定 404 的请求，报错后可让 `fetch_daily_data` 记录本源失败并切到下一个源。新增回归 6 例（605/689/900 → `.SS`、001/003/200/301 → `.SZ`、未知段抛错），反向验证：只还原前缀表则前 5 例中 4 例转红

- [修复] Tushare 旧版实时行情的成交量被多除了一次 100：`TushareFetcher.get_realtime_quote` 在 Pro 接口 `quotation` 失败后的降级分支里写的是 `volume=safe_int(row['volume']) // 100  # 转换为手`，而该分支直连新浪 hq 行情源（vendored tushare `stock/trading.py:353` 的字段说明原文为「8：volumn，成交量 maybe you need do volumn/100」，即原始值就是「股」，除以 100 才得到「手」），于是这条分支吐出的成交量比统一契约小 100 倍，且与同文件日线侧「`vol` 手 → ×100 → 股」的口径互相矛盾；现移除该除法并补注释说明。新增回归 1 例（断言旧版分支的 volume 与接口原值一致、且不等于原值 / 100），反向验证：把 `// 100` 改回去该例转红。未改动同一方法里的 Pro 分支（`safe_int(row.get('vol'))`）——该接口的 `vol` 单位无法离线确认（本机无法访问 tushare.pro，官方文档亦未落在可抓取范围内），按「先验证再修」留待联网核实，已在 `docs/data-source-stability.md` 的成交量单位契约表里标为「待联网确认」

- [修复] A 股成交量单位统一为「股」：东方财富口径（efinance 与 akshare 的 `*_em` 接口，日线与实时行情同源）返回的成交量本是「手」，此前未换算就写入 `stock_daily.volume`（该列注释为「成交量（股）」），并与「股」口径的实时行情直接相除，导致 `volume_change_ratio` / `volume_ratio_5d` 稳定放大 100 倍——实测同一交易日`volume_ratio_5d = 1.00 → 100.00`（`量能正常` → `放量上涨`），LLM 提示里固定出现「成交量较昨日变化约 100.00 倍…量能信号必须降权解读」，且日线单独展示时成交量被低估 100 倍；现换算收敛到 `data_provider/realtime_types.py` 的 `EM_VOLUME_LOT_SIZE` / `em_lots_to_shares`，在 efinance 日线（股票与 ETF 共用写入口）、akshare 东财日线（`stock_zh_a_hist`）、东财 ETF 日线（`fund_etf_hist_em`）与两处东财实时行情（efinance `get_realtime_quotes`、akshare `stock_zh_a_spot_em` / `fund_etf_spot_em`）统一换算；单位依据来自 efinance 自带文档字符串中的真实样本（600519 2021-07-29：成交量 63864 / 成交额 1.129957e10 / 换手率 0.51%，按「手」解读换手率恰为 0.51%，按「股」则只有 0.0051%），并补齐了上一轮「归一腾讯实时行情为股」只改了实时侧、日线侧仍为「手」的缺口。未改动：新浪/腾讯历史接口、baostock、Tushare 日线、TickFlow（本就为「股」或已换算）；pytdx 的 `vol` 单位需联网探测，本机无法验证，未改。新增 `tests/test_volume_unit_contract.py` 10 例（含「共享 `_normalize_data` 不得重复放大新浪/腾讯」的护栏与跨源比值断言），反向验证：撤掉日线换算后 4 例转红
- [修复] `/api/v1/stocks/parse-import` 是 `async def` 却在事件循环线程上同步跑完整解析，单个未鉴权请求即可让整个后端停止响应：处理函数体内除 `await request.json()` / `await request.form()` 外没有任何 await 或线程池切换，而 `parse_import_from_text` / `parse_import_from_bytes` 完全同步——`_parse_dataframe` 对每一行「只有名称、没有代码」的记录调用 `resolve_name_to_code`，其内部按序执行本地反查表、pypinyin 全表比对、**同步 AkShare 网络请求**（`ak.stock_info_a_code_name()`）与对全量 5000+ 名称的两轮 `difflib` 模糊匹配。服务是单进程单事件循环（`server.py` 与 `main.py` 的 uvicorn 均未配置 workers），因此这段耗时期间 `/health`、问股 SSE 分析流与所有轮询接口全部排队；上传上限是 2MB 文件 / 100KB 文本，未启用管理员认证（默认）时任一外部客户端都可触发。现把两处解析改为 `await asyncio.to_thread(...)`。实测（本机，AkShare 已预热）：200 行纯名称粘贴在修复前让事件循环连续阻塞 **6.37 秒**（心跳协程 4 次 tick 后停顿 6.37 秒），修复后同一请求仍耗时 5.52 秒但事件循环最大停顿降到 **0.12 秒**、tick 数 10 → 253；1MB / 14 万行纯代码 CSV 上传的剩余循环侧开销（multipart 解码 + ≤2MB 同步读 + 14 万条响应序列化）实测最大停顿 0.22 秒，属体量上限内的量级，未改动。新增回归 3 例（JSON 分支与 multipart 分支的解析调用线程 id 必须不等于事件循环线程 id；超限文本仍返回 400 `too_large`）。反向验证：把两处调用改回直接调用，前两例转红（`assert 8444207488 != 8444207488`），恢复后转绿；`tests/test_stocks_parse_import_api.py` + `tests/test_import_parser.py` + `tests/test_stock_watchlist_api.py` 34 例通过，flake8 告警集与 HEAD 逐项一致（均只剩既有 13 处 W293）。文档：`docs/full-guide.md` 的「智能导入」补一句解析在工作线程执行、不阻塞其它请求。
- [修复] 问股流式接口的消费侧等待时限与整轮预算脱钩，会先发「分析超时」而后台仍在跑并落库：`api/v1/endpoints/agent.py` 的 SSE 消费循环对非 Codex 后端写死 `asyncio.wait_for(queue.get(), timeout=300.0)`，而运行时预算是 `AGENT_ORCHESTRATOR_TIMEOUT_S`（默认 600 秒，由 `AgentChatExecutor.timeout_seconds` 作为 `max_wall_clock_seconds` 交给 runner，到点后运行时会自己发终态事件）。默认配置下 API 侧在 300 秒先到期：流上发出 `error_code=timeout` 的「分析超时」并结束本轮，但 LiteLLM 后端不可取消（`run_sync` 只在 `codex_app_server` 时注入 `cancel_event`），worker 线程继续跑完 `execute_turn`，而 `chat_executor` 无论成功还是失败都会 `add_message(session_id, "assistant", ...)`——用户看到的是超时，重新加载会话却出现一条从未在流里到达的回答；文档本身也写着同步分析「约需 2–5 分钟」，300 秒对正常长回合本身就偏紧。现改为由同一来源派生：新增 `_resolve_stream_wait_timeout(executor)` 读取执行器实际使用的预算并加 30 秒收尾余量（`_STREAM_WAIT_GRACE_SECONDS`，只覆盖运行时自己的终态事件送达与协作式预算检查粒度，不是第二份预算），使预算自己的终态事件总是先到、二次上限退化为「运行时长已超出自身预算」时的兜底；`AGENT_ORCHESTRATOR_TIMEOUT_S=0` 关闭预算时不叠加二次上限，与 Codex 分支「后端独占唯一截止时间」的既有约定一致，该分支代码保持不变。超时兜底触发时新增 warning 日志，说明该后端的 worker 仍在运行且可能随后落库——这条路径无法在不引入「LiteLLM 可取消」这一更大改动的前提下彻底消除，因此改为可诊断而不是静默。新增回归 10 例（`600/300/60` 秒预算分别派生 `630/330/90`、且派生值始终大于预算；`0`/`None`/属性缺失/非法字符串/负值一律不设二次上限；端到端断言 litellm 流实际传给 `asyncio.wait_for` 的是 `630.0` 而非 `300.0`；Codex 流不引入任何二次上限）。反向验证：把调用点改回写死的 `300.0`，`test_litellm_stream_wait_limit_follows_the_runtime_budget` 转红，恢复后转绿；`tests/test_agent_chat_api.py` + `tests/test_agent_sse_cleanup.py` 50 例通过，flake8 告警集与 HEAD 逐项一致（无新增）。文档：`docs/LLM_CONFIG_GUIDE.md` 与 `docs/LLM_CONFIG_GUIDE_EN.md` 的「默认模型问股 / LiteLLM 配置兼容说明」各补一条派生契约。
- [修复] 三个 Agent 的模型数值字段（风险分/持仓风险分/情绪分）会让整个 stage 失败或被静默改写：这些字段直接进 `AgentOpinion.confidence` 的换算或整数比较，而模型对 prompt 里的数值槽位并不守约（`null`、`"-"`、`"高"`、带引号的 `"8"` 都很常见）。`RiskAgent.post_process` 的 `float(parsed.get("risk_score", 50))` 只在**键缺失**时用默认值，`risk_score: null` 抛 TypeError、`"高"` 抛 ValueError；异常被 `BaseAgent.run` 的通用 `except Exception` 吞掉，丢掉的是整个 风险面 stage（不是那一个数字）——没有 risk opinion，`build_risk_override_plan` 看到 `evidence_present=False`，风险否决/降级对这次运行静默失效，流水线却照常输出一份看起来完整的结论。`PortfolioAgent.post_process` 把 `portfolio_risk_score` 原样拿去做 `<= 3` / `>= 7` 比较，`"8"`/`null` 同样抛 TypeError 并丢掉 持仓 stage（连该方法自己那条 0.3 置信度的 hold 兜底都拿不到，比「JSON 解析失败」路径失败得更彻底）。`DecisionAgent.post_process` 则是反向问题：`dashboard.get("sentiment_score", 50) or 50` 用 `or` 兜底，把合法的 `0`（最看空）抬成 50% 置信度，runtime facts 与分歧解释里的置信度因此失真。现新增共享助手 `protocols.normalize_model_number(value, default, minimum, maximum)`（缺失/非法/`bool`/非有限值一律取 `default`，再按需夹到区间），三处改为走它：风险分默认 50 夹到 0-100、持仓风险分默认 5 夹到 1-10、情绪分默认 50 **不夹**——越界分数要继续让 `confidence` 落在 0-1 之外，从而被 `confidence_input_valid=False`排除在采样之外，夹成 0.0 等于把一个伪造的置信度混进样本。新增回归 6 例（风险分 `null`/`"高"`/`"-"` 仍产出 opinion 且回落到 0.5、越界风险分夹到 0/1、持仓风险分 `"8"`/`"3"`/`"high"`/`null` 分别得 sell/buy/hold 且 stage 不丢、情绪分 `0` 得 0.0 且仍可采样、非数值回落 0.5、越界分数保持不可采样），反向验证：`"8"`/`"high"`/`null`/`None` 等 7 个子例在修复前全部转红（TypeError）、`sentiment_score: 0` 得 0.5，修复后转绿；`tests/test_multi_agent.py` 221 例通过，`tests/ -k "agent or risk or decision or portfolio"` 1468 例通过，flake8 仅剩 `risk_agent.py` HEAD 即有的 1 处 W391

- [修复] 问股正文里的裸 5 位数字被当成港股代码并静默切股：`stock_scope.extract_stock_codes` 的第 5 条模式 `(?<!\d)\d{5}(?!\d)` 会命中任何裸 5 位数字，`_normalize_tool_stock_code` 再把它补成 `HK<5位>`，于是「看看 600519 的成交量 12000 手」被判成切股到 `HK12000`、「分析 600519，目标价 25000」切到 `HK25000`、「研究 600519 市值 21000 亿」切到 `HK21000`——`_switch_context` 随即清空当前 `stock_code`/`stock_name` 与全部按标的缓存的分析（`previous_analysis_summary`、`previous_strategy`、`realtime_quote` 等），再拿一个不存在的代码去查数据；Web 侧 `chatStockCode.ts` 的自由文本提取用 `\b(\d{5,6})\b` + `validateStockCode` 的 `^\d{5}$` 分支镜像了同一语义，页面会把同一个假标的高亮成「当前标的」，与后端实际使用的标的分叉。现两侧都只把**零填充的规范写法**（`00700`/`01810`，即本项目 `HK00700` 的补零形态）当作无标记裸 5 位数字的港股代码，带 `hk`/`.HK` 标记的写法（`hk81200`、`81200.HK`）不受约束，A 股 6 位代码与美股/日股/韩股模式也不受影响；区分依据是「中文里裸 5 位数字同时是价格/成交量/金额的常见写法，而港股代码的规范写法必然补零」，代价是非零填充的港股代码（如人民币柜台 `80700`）必须带标记书写。新增回归 4 例（成交量/目标价/市值/现价四类句式保持 `maintain` 不改上下文）+3 例正向（`00700` 仍切股、`hk81200`/`81200.HK` 仍切股、`比较 600519 和 00700` 仍比较）与前端 `chatStockCode.test.ts` 4 例，反向验证：修复前 4 例全部转红（`mode` 为 `switch`、`allowed_stock_codes` 为 `{'600519','HK12000'}`、前端提取出 `HK12000`），修复后转绿；`tests/test_multi_agent.py` 215 例、`tests/test_agent_executor.py`+`tests/test_image_stock_extractor_litellm.py` 108 例通过，前端 `npx vitest run` 1316 例通过、`tsc --noEmit` 与 `eslint` 无输出，flake8 无告警

- [修复] 深度研究在「一条来源都没拿到」时仍报告成功，并让模型凭空合成一份报告：`ResearchAgent.research` 把每个子问题的结果无条件 `all_findings.append(finding)`，而失败结果同样是完整 dict（`{"content":"","success":False,"error":...}`），于是合成分支的 `if all_findings` 恒为真、`else {"content": "No findings gathered."}` 这条兜底永远不可达；`_synthesise_report` 内部再按 `for f in findings if f.get("content")` 过滤，交给模型的 findings 块**是空的**，却仍要求它按「Executive Summary / Key Findings / Risk Factors / Conclusion」写出完整报告；`success=not report.get("error")` 随之判为成功，`findings_count=len(all_findings)>0`，机器人与 API 都按成功路径渲染，用户拿到一份措辞笃定、零来源支撑的研究报告（搜素 Key 失效、所有子问题都报错时必然触发）。现只把**有内容**的子问题结果计入 findings（无内容的一律记 warning 日志，让「模型没产出」与「工具失败」可区分），并在一条可用来源都没有时返回带 `error` 的失败结果（`error="no_usable_findings"`），不再调用合成；`findings_count` 随之表示真实可用来源数（机器人正是按 `Sources: {findings_count}` 渲染，语义由此变准）。新增 3 例回归（全部子问题失败→不合成且 success=False、部分成功→只把有内容的那条交给合成且 findings_count=1、「成功但无内容」也不算来源），反向验证：3 例在修复前全部转红（合成被调用、success=True、findings_count=3）；顺带修正 1 例既有用例的 fixture——`test_research_marks_synthesis_fallback_as_failure` 的子问题桩返回 `{"summary":"done"}` 不带 `content`，本意是验证「合成返回 error 时 success 必须为假」，此前靠「任何结果都算 finding」的旧行为才走到合成，现补上 `content` 让该用例回到它声明要验证的路径；`tests/test_multi_agent.py` 与 `tests/test_agent_chat_api.py` 249 例全绿，`tests/ -k "research or agent or bot"` 921 例通过，flake8 仅剩该测试文件 HEAD 即有的 2 处告警（E303 / F841）
- [修复] RiskAgent 输出字符串 `"false"` 时被当成「风险否决」，把 buy 降级成 hold：`build_risk_override_plan` 用 `bool(risk_raw.get("veto_buy"))` 取真值，而 `risk_raw` 是 RiskAgent 的原始 LLM JSON、未经归一化——prompt 里 `veto_buy` 声明为 `true|false` 布尔槽位，但模型输出字符串 `"false"` / `"no"` / `"0"` 很常见（也实测复现），非空字符串恒为真，于是 `veto_buy=True`、`evidence_present=True`，`build_risk_override_application` 返回 `applied=True`、`post_risk_signal=hold`，`AgentOrchestrator._apply_risk_override` 随即把 `decision_type` 从 buy 改写成 hold，并在报告/历史里记下 `risk_override_applied={"from":"buy","to":"hold","adjustment":"veto","reason":"risk_veto"}`。后果：一份「低风险、显式不否决」的风险报告会让用户看到一个被否决的买入决策和「风险否决」理由，与实际风险判断相反，且无法从界面分辨真假否决。现改用仓库既有的布尔归一化助手 `_coerce_bool`（识别 `1/true/yes/on` 为真、`0/false/no/off` 为假）；**无法识别的取值仍按「有否决」处理**，风险控制宁可少买也不漏判（不做成「无法识别即放行」）。新增 3 组参数化回归（5 种「不否决」写法不触发降级、6 种「否决」写法仍触发、无法识别的取值保持保守否决），反向验证：修复前第一组 5 例全部转红（`applied` 实得 True），修复后 14 例全绿；`tests/ -k "risk or agent or orchestrator or disagreement"` 847 例通过，flake8 0 告警
- [修复] 券商 CSV 导入的去重因子用「绝对行号」，同一批成交在重新导出的文件里整批重复入账：`parse_trade_csv` 把 `_source_line_number = int(idx) + 2` 写进记录，`_build_dedup_hash` 又把它拼进 payload，而 DB 侧的唯一性只由 `trade_uid` / `dedup_hash` 判定（`source_line_number` 对「同一文件原样重导」稳定，对「重新导出的文件」不稳定）。券商重新导出必然改变行位置——顶部多一行查询区间说明、删掉一行已撤销成交、按日期重排、只导出其中一段——于是整份对账单的 hash 一起变，`has_trade_dedup_hash` 判不出重复，全部作为新成交走 `record_trade` 落库：持仓数量与成本、现金流水按整份对账单重复累加（同一笔 100 股买入记两次 → 持仓 200 股、成本翻倍、现金少扣一次），已实现/未实现盈亏与风险指标随之失真，且重复记录落库后无法靠「再导一次同一文件」纠正（新 hash 已写入，只能手工删交易）。现把位置因子换成「该内容在本文件内第几次出现」（`_duplicate_ordinal`，内容键抽成 `_dedup_content_key` 供 hash 复用）：重排、增删行、只导一段都得到同一个 hash，同时仍能区分同字段的合法分笔成交（第 1 次与第 2 次），与原设计说明（CHANGELOG 1520「dedup hash 纳入行序号作为区分因子，确保同字段合法分笔成交不被误折叠」）的意图一致。新增 1 例回归（先导入 2 行，再导入「顶部插入一笔更早成交」后的同一份对账单，断言只新增 1 笔、2 笔判重、600519 的成交数量为 5 与 10 而非被重复累加），反向验证：修复前该例转红（`inserted_count` 实得 3），修复后 `tests/test_portfolio_pr2.py` 26 例全绿，flake8 按 CI 选择集（E9,F63,F7,F82）0 告警。**升级影响**：新老 hash 算法不同，升级前已导入的记录不会被新算法识别——升级后重导**同一个文件**也会再入账一次，需手工删除；若要求无缝迁移，需要额外保留旧算法的 hash 作为兜底查重键（本次未做，属额外契约，需拍板）
- [修复] 调度器的自动补跑判据永远不成立，跨交易日缺口实际不会自愈：`_run_analysis_locked` 的成功分支先执行 `self._last_success_at = datetime.now().isoformat()`（记录本次成功、用于失败告警与状态接口），紧接着才调 `_maybe_trigger_backfill(stock_codes)`，而后者算缺口用的是 `self._trading_day_gap_since(self._last_success_at)`——读到的正是刚被覆盖成本次成功时间的那个值，`_trading_day_gap_since` 又传 `market=None`、`MARKET_EXCHANGE.get("")` 为假值，`get_effective_trading_date` 对它 fail-open 回落到市场本地自然日（今天），于是 gap 恒在 {-1, 0, +1} 之间，判据 `gap > runtime_backfill_max_days`（默认 1）永不成立。后果：服务商发行日/长假期导致「本机多日没跑成功」时，`_run_analysis_locked` 恢复后的第一次运行不会去补那几天的数据，而 `_last_run_at` / 状态接口看起来一切正常，用户只能靠手工补跑；原有 3 例测试全是直接调 `_maybe_trigger_backfill(None)` 并手工预设 `_last_success_at`，绕过了真实调用点，所以这条死代码一直没被发现。现改为在覆盖前取出 `previous_success_at` 并作为参数传入（为 None 时回落到实例字段，保持直接调用可用），缺口基准回到「上一次成功」；顺带按报告指出的次要风险夹住 `runtime_backfill_max_days` 的下界（`max(0, ...)`，负数会让 `gap=0 > max_days` 成立并把补跑无限递归下去），并加 `_backfill_in_progress` 闸门把补跑限制在至多一层——嵌套的那次运行会以「刚刚这次的成功时间」为基准再判一次缺口，本身就是天然的终止条件，闸门是防止将来判据被改动后退化成递归。新增 4 例回归，其中 2 例走真实入口 `_run_analysis_locked`（首次成功后距上一成功 4 天、`max_days=1` 时必须跑第二次；首次成功无历史基准时不得补跑），1 例把缺口函数钉成恒为开放以证明补跑不递归（无闸门时该例会一路 RecursionError），1 例验证负数配置被夹到 0 且不多跑；反向验证：把源码换回 HEAD 后前两例转红（`assert len(runs) == 2` 实得 1），把 `max(0, ...)` 去掉后第 4 例单独转红，恢复后 7 例全绿；scheduler 相关 4 个测试文件 35 例通过，flake8 仅剩该文件与测试文件 HEAD 即有的 4 处 E3xx 告警（CI 只 select E9,F63,F7,F82，不阻断）
- [修复] 历史 K 线的 DB 命中条件在非交易日必然失配，本地已有历史也照样走网络：`load_history_df` 在无 `target_date`、无 frozen ContextVar 时取 `end = date.today()`，命中条件是 `bars and latest_date >= end and len(bars) >= required_records`，而 `db.get_data_range` 是闭区间，所以它实际要求「库里存在一根日期正好等于今天的 K 线」。周末、节假日、盘中、以及当日日线同步尚未跑完时，库里的最新一根 K 线一定早于今天，于是即使本地已有整段历史也会跳过缓存去走网络；网络被限流或不可用时返回 `(None, "none")`，`_refresh_price_history` 便不写缓存，`GET /portfolio/positions/{symbol}/price-history` 每次都返回 `{"source":"refreshing","items":[]}`，持仓价格走势图长期为空且一直显示刷新中、不会自愈。现改为按标的所属市场取「最近一个已完成交易日的日期」（复用 `src/core/trading_calendar.get_effective_trading_date`，与模拟盘估值用的是同一套日历；日历不可用时 fail-open 回落到自然日，行为与原先一致），抽成 `_latest_session_date` 便于测试。新增 1 例回归（把已完成交易日固定为「今天往前 7 天」并种子 40 根截止该日的 K 线，断言走 DB 缓存、查询上界等于该交易日、且完全没触发网络回落），反向验证：修复前该例转红（`'none' != 'db_cache'`，DB 路径被整体跳过），修复后 7 例全绿；`tests/ -k 'history or loader or price_history or screening or data_tools'` 415 例通过，flake8 无告警
- [修复] 图片提取结果里带 `.HK` 的港股被静默丢弃：`_normalize_code` 只剥离 `.SH` / `.SZ` / `.SS` 后缀，既不认 `.HK` 后缀也不认 `HK` 前缀，带这两种写法的代码走到最后 `return None`；而 `_parse_items_from_text` 的对象数组分支对 `code is None` 的条目直接 `continue`，既不记日志也不回报，整条 item（含 name / confidence）消失。截图里的港股常按原文写成 `00700.HK`，模型也常照抄，于是用户上传含港股的持仓/自选截图时，提取结果里港股全部缺失且没有任何提示，同一张图重复上传结果一致、无法自愈。实测修复前 `[{"code":"00700.HK",...},{"code":"600519",...}]` 只解析出贵州茅台一条。现补上 `.HK` 后缀与 `HK` 前缀两种形态，统一成 5 位纯数字码（与既有 5 位数字分支的输出形态一致，`700.HK` 按 `zfill(5)` 补成 `00700`）；顺带给对象数组里被丢弃的条目加一条 debug 日志，让「模型没认出」与「代码形态不支持」在日志里可区分。**顺带修掉同源的一处假代码**：裸的 `HK` / `SH` / `SZ` / `BJ` / `SS` 会被 1–5 个字母的美股规则接受，它们只可能来自交易所后缀被拆开（`stock_scope._EXCHANGE_TOKEN_CANDIDATES` 出于同一原因拒绝这几个词），现加入 `_FAKE_CODES`。新增 3 例回归（`.HK`/`HK` 前缀归一到 5 位数字、对象数组里的港股条目不丢、裸交易所代号不是代码），反向验证：3 例在 HEAD 上全部转红（`_normalize_code('00700.HK')` 为 None、腾讯控股整条丢失、`['HK','SH']` 混进结果），修复后转绿；`tests/ -k 'image or extract'` 280 例通过，flake8 无告警
- [修复] 图片提取股票的兜底解析把英文单词与 JSON 字段名当成美股代码返回：`_parse_items_from_text` 只有在解析出「对象数组且至少一条有效」时才返回，否则回落到 `_parse_codes_from_text(text)`，而该兜底用 `re.finditer(r"\b([0-9]{5,6}|[A-Z]{1,5}(\.[A-Z])?)\b", text, re.IGNORECASE)` 直接在**原始响应文本**上扫字母，`_normalize_code` 的 `^[A-Z]{1,5}(\.[A-Z])?$` 又把任何 1–5 个字母判为合法美股代码，`_FAKE_CODES` 只挡了 7 个硬编码词。于是模型没严格按 prompt 返回对象数组时，用户会拿到假代码——实测 `I could not find any stock codes in this image.` 产出 `I / COULD / NOT / FIND / ANY / STOCK / CODES / IN / THIS / IMAGE` 十项，`{"codes": ["600519"], "names": ["贵州茅台"]}` 产出 `CODES / 600519 / NAMES`，`00700.HK`、`600519.SH` 这类带交易所后缀的写法还会额外产出 `HK` / `SH`。这些代码经 `extract_stock_codes_from_image` 原样进入 `ExtractFromImageResponse.codes/items`，用户勾选后被当成股票送进分析队列，得到一批无意义或报错的结果，真正的代码反被噪声淹没。现把兜底拆成两个显式形态：数字串仍按 `\b[0-9]{5,6}\b` 扫（`600519.SH` 里的 `600519` 照旧能取到）；字母代码改成大小写敏感且前后不挨词字符或点号（`(?<![\w.])[A-Z]{1,5}(?:\.[A-Z])?(?![\w.])`），因此小写英文单词与交易所后缀不再入列；此外当「一个数字代码都没有且文本里出现 ≥2 个连续小写字母」时判定为自然语言回复，整段跳过字母扫描（宁可返回空列表，也不返回假代码）。新增 5 例回归（英文拒绝语返回空、对象响应只取真实代码、交易所后缀不被当成代码、中文混排仍能取出 600519/300750/AAPL、不含数字代码的全大写清单仍能解析），反向验证：前 3 例在 HEAD 上转红（分别产出 10 项假代码、`CODES/NAMES`、`SH`/`HK`），后 2 例是防回归护栏，修复后 42 例全绿；`tests/ -k 'image or extract'` 277 例通过，flake8 无告警
- [修复] 模拟盘 bar 缓存把「当天 bar 还没入库」缓存成「当天已覆盖」，当天估值、止损止盈与成交补做在进程存活期内全部失效：`_load_bars` 用 `end = date.today()` 作为查询上界，却把 `(start, end, bars)` 整个写进模块级 `_BAR_CACHE`，命中判据是纯日历比较 `start <= as_of <= cached_end`——完全不看 `bars` 里是否真有 `as_of` 这一天的行。于是「当天 bar 还没落库时先读过一次」会把窗口记成已覆盖当天，此后同一进程内每次请求当天 bar 都命中缓存拿到 None（`_BAR_CACHE` 无 TTL、无失效接口，只有 1024 条 LRU 淘汰）。触发路径是交易日盘中启动服务：scheduler 的 `paper_daily_valuation` 后台任务 `run_immediately=True` 立刻跑一次，此时 `resolve_valuation_date` 返回上一交易日，缓存就写成了 `(昨日-365, 今天, 不含今天的 bars)`；当天收盘后再跑时 `resolved` = 今天 → 命中缓存 → None。后果有三：(1) `_valuate` 打印「无 bar，估值跳过该标的」并 `continue`，但 `_record_snapshot` 仍写当日快照，当天净值 = 当日现金 + 上一交易日市值，当天涨跌完全不体现；(2) 当天 bar 内的止损/止盈没有被判定，而快照已落库，调度器的 `resolved <= latest_snapshot_date` 守卫让这一天不再重算，用户在 Web 上点「刷新」（`force=True`）也仍被缓存挡住返回 None；(3) `_open_or_add` 对当天信号返回 `data_unavailable` 且不记账（可重试语义），等收盘 bar 落库后用户按提示做「历史回填」时 `backfill_history` 仍拿到 None 并计入 `signals_unavailable`，这笔成交在本进程存活期内永远不会补上。现把缓存窗口的上界记成 `bars` 里真实存在的最新日期（`max(bars)`），而不是查询用的 `end`：请求的日期晚于已加载的最新 bar 时会重新查库，当天 bar 一旦落库就能被取到，刷新按钮与历史回填两条补救路径随之恢复。新增 1 例回归（先种子昨日 bar 读一次、再种子当天 bar、同一进程内请求当天必须拿到当天 bar），反向验证：修复前该例转红（`assert bar is not None` 失败，缓存返回 None），修复后转绿；`tests/ -k 'paper'` 70 例通过，flake8 仅剩该文件 HEAD 即有的两处既有告警（E741 / F841）。**未随之改动的一处**：`_valuate` 在某个持仓取不到 bar 时只 `continue` 并照常落快照，报告建议「取不到当日 bar 就不落快照、不推进 `latest_snapshot_date`」。没有照做是有意的——「停牌/退市等长期无 bar 的标的」与「当天 bar 暂时还没入库」在数据上无法区分，改成「有一个标的没 bar 就整日不落快照」会让持有这类标的的账户永久停止估值，比现状更糟。本次只修掉缓存这条**确定性**的成因；「当天 bar 尚未入库的窗口内落了一次快照」这一竞态仍在，需要产品决策（例如加一个「bar 缺失容忍窗口」或按 available/expected 比例判定）后再动。

- [修复] 历史日期的组合快照用实时行情缓存的「当前价」估值并落库，把回撤与净值曲线改成了今天的价格：`_resolve_position_price` 的 `if include_realtime:` 分支完全没有比较 `as_of_date`——只要该 symbol 在 `position_quote_cache` 里有行就直接返回它（`get_latest_cached_quote` 每 symbol 只有一行、取的是最近一次抓取，没有日期维度），`price_date` 记成缓存里的 `quote_date` 或今天；只有缓存缺失时才回落到 `stock_daily` 历史收盘。而 API 文档写的是「是否让**今天**的快照/风险快照优先用实时价」，`_build_positions` 里触发后台刷新的 `active_symbols` 也确实只按「`as_of_date == date.today()`」收集——只有读取这条路径漏了这个判据。于是当天打开过一次持仓页（后台刷新会写缓存）之后，回撤监控的 `_ensure_drawdown_snapshot_window` 对最近 30 个缺失日逐日调`get_portfolio_snapshot(as_of=过去某天, include_realtime=True)`，这些历史日期的市值全部按今天的价格算出来——而它不是只算给响应看的，`replace_positions_lots_and_snapshot` 会把 positions / lots / daily snapshot 三张表一起写库，之后的请求又按「该日已存在」跳过，错误行不会自愈。后果是最大回撤 / 当前回撤 / 回撤告警阈值判定失真，且界面上看起来很正常（`as_of` 是过去某天，而该持仓的 `price_date` 是今天、`price_stale=false`）。现把实时缓存分支限定为「`include_realtime` 且 `as_of_date == date.today()`」，历史日期一律走 `stock_daily` 收盘价。新增 1 例回归（as_of 取 5 天前，当天收盘价 110.0、缓存里是今天的 125.0，断言 `price_source == "history_close"` 且市值按 110.0 计），反向验证：修复前该例转红（拿到 `realtime_cached` / 125.0），修复后转绿；`tests/ -k 'portfolio or drawdown or risk'` 232 例通过，`tests/test_portfolio_service.py` 36 例全绿，flake8 仅剩该文件两处 HEAD 即有的既有告警（E123 / F841，CI 只 select E9,F63,F7,F82 故不阻断）
- [修复] 组合页同时提交两个持仓的「分析」时，先返回的那个会把另一个的「提交中」清掉：在途状态只有 `positionAnalysisLoadingKey` 一个字符串，`handleAnalyzePosition` 进函数就无条件覆盖它、`finally` 里又无条件 `set(null)`，而行内按钮的禁用只看「本行 key 是否等于这个单值」。于是「点 A 行分析 → 点 B 行分析」之后，任一请求先落地都会走 `finally` 把单值清空——此时另一个请求其实还在途，但两行的按钮都恢复成可点的「分析」，用户会以为前一次提交没生效（或压根没点中）而重复提交同一个持仓的分析，后端就在同一份持仓上叠出两个分析任务。同页三个录入表单此前已有各自独立的提交锁，只有这里退化成了单值。现改为按行 key 的集合（`Record<string, true>`）：提交时只置位本行，`finally` 只删除本行自己的 key，行内按钮与详情弹窗的「提交中...」判据同步改成查本行 key。新增 1 例回归（两个持仓行，第一个的分析请求挂在永不结束的 pending 上，点第二行后断言两行都仍是禁用的「提交中」），反向验证：修复前该例转红（第一行按钮已恢复成可点的「分析」），修复后转绿；全量 299 文件 / 1312 用例全绿，`tsc --noEmit` / `eslint` / `ui_governance` 通过
- [修复] 设置页每个没测过的 LLM 渠道都挂着一个「测试中」徽标：渠道行头部的状态徽标条件是 `testState?.status !== 'idle'`，而 `testStates` 是一张稀疏表——没测过的渠道根本不在里面，`testState` 为 `undefined`，`undefined !== 'idle'` 恒为真，于是三元表达式的兜底分支把「测试中」渲染了出来（`statusVariant` 同时取 `default`，是个灰色徽标）。结果是打开设置页时所有渠道都显示「测试中」，用户无从判断哪些真的在测；这条分支的本意是「有状态且不是 idle 才显示」。现改为先判 `testState` 存在再比状态。新增 1 例回归（三个渠道、无任何测试时，未测试的渠道行不得含「测试中」），反向验证：修复前该例转红（`▶omegaopenai1 个模型已配置测试中✕`），修复后转绿；全量 122 文件 / 1311 用例全绿
- [修复] 设置页 LLM 渠道「测试连接」的结果会落到没测过的渠道上：`handleTest` 用**点击时的数组下标**作 key 存放测试状态（`testStates[index]`），既没有 request-id/nonce 守卫也没有卸载检查，而 `removeChannel` 在删掉别的渠道时会把下标整体平移（它虽然会 `setTestStates({})` 清空，但清不掉此后才返回的在途响应）。于是「点 B 行的测试连接 → 响应返回前删掉 A 行」之后，B 的结果会按旧下标写进「删掉 A 之后占据该下标的那个渠道」（三行 [A,B,C] 的情形下即 C），C 行显示绿色状态点与「连接正常」徽标，用户据此认为从未测过的 Key / BaseURL 已验证可用；若响应更慢、发起测试的渠道本身被删掉，结果则被静默丢弃（转圈消失）。同文件里 `handleDiscoverModels` 与 `handleCapabilityCheck` 早已用 `channel.id` 作 key 并带 nonce 守卫，只有测试这条路径没有。现把 `testStates` 改为按 `channel.id` 存放，并补 `testNonceRef` / `testRequestIdRef`：响应回来时校验 nonce（配置被改、渠道被删都使 nonce 失效），删除渠道时只清理它自己的状态而不是清空全部。新增 1 例回归（三个渠道中测中间那个，在途时删掉第一个，响应返回后断言从未测试过的渠道没有「连接正常」、发起测试的渠道有），反向验证：修复前该例转红且失败信息正是「omega 收到 连接正常」，修复后转绿；全量 122 文件 / 1310 用例全绿，`tsc --noEmit` / `eslint` 通过
...
- [改进] 问股提案通道统一为单一 `action_proposal` SSE 事件（携带 `kind` / `summary` / `proposal`），告警提案一并迁入，取代此前的 `alert_proposal`；前端确认卡片与 bot 提示行随之泛化，卡片支持一条消息多张提案、失败后仍可重试或取消
...
- [新功能] 问股 Web 输入框支持贴图（粘贴 / 拖放 / 选文件，jpg/png/webp/gif，上限 2MB）：图片二进制**不落盘**、不进消息历史，而是在进入 agent 之前先由视觉模型读一遍（新增 `src/services/chat_image_context.py` 的 `build_image_context`，其 prompt 带上用户原话以保证摘要答对问题），结果拼成 `【图片内容】…【用户问题】…` 文本块注入当轮用户消息——注入点在 `api/v1/endpoints/agent.py` 两个端点的函数体内、`prepare_turn` **之前**，因此注入后的文本自动进 `conversation_messages`，而 agent 主模型、`llm_adapter`、`executor`、runner 与工具循环一行未改；视觉失败一律 **503 + 固定中文文案**（上游异常只进脱敏日志），**不降级成「看不到图也照样回答」**——那种回答既无用又会伪装成成功，且前端会按关键词（`timeout` 等）把视觉超时误报成「请检查网络与代理」。配套：两套 chat 提示词新增规则 10，**只有**用户明确表达「把图里这些加入自选/关注」才用 `propose_watchlist_change` 逐个提案，仅因图中出现代码不得主动提案（Codex 只读面不加该规则）；只贴图不打字也受支持（`message` 可为空，空且无图时显式 400），图片经 `meta.imageDataUrl` 仅在会话内渲染、刷新即失；刷新后侧边栏标题剥掉注入脚手架（不再显示 `【图片内容】…`），用户气泡把注入块折叠成「[图片] 已由 AI 读取」而不是把整段当用户的话倒出来，活跃标的上文解析也只取 `【用户问题】` 之后的文本（否则块里 ≥2 个代码会让还原后的会话静默切进对比模式）。新增 38 例后端回归（`tests/test_chat_image_context.py`、`tests/test_agent_chat_image_request.py`、`tests/test_chat_prompt_image_rule.py`、`tests/test_chat_image_vision_routing.py` 的离线部分，另有 1 例网络守卫）与 22 例前端用例（ChatPage / agentChatStore）；后端图片失败路径统一走既有脱敏，`docs/architecture/api_spec.json` 已按运行时契约重新生成
- [修复] 截图提取股票代码此前静默失效：litellm 的 provider 专属路由（`deepseek/`）不会转发 `image_url` 内容块，模型「看不见图」却仍返回**合法但无用**的回答（`[]` 或「未收到图片」），接口回 200，用户与日志都无从察觉；`litellm.register_model(..., supports_vision=True)` 实测无效，能力元数据也判断不了，因此没有任何运行期检查能抓住这类失败。`src/services/image_stock_extractor.py` 新增 `_PROVIDER_ROUTES_WITHOUT_IMAGE_FORWARDING`（**只收实测过会剥图的路由**，不凭猜测添加）与 `_vision_wire_model()`：命中则改走 litellm 通用 `openai/` 兼容路由，`api_base` 仍按同一 deployment 传入，因此打的还是同一个 endpoint；`_call_litellm_vision` 泛化为 `_call_litellm_vision_with_prompt(prompt, ...)` 并保留原名与原签名做薄包装，设置页提取路径与既有 mock 测试因此一行未改。新增 `tests/test_chat_image_vision_routing.py`，其中 `test_vision_call_actually_sees_the_image` **不 mock 转发层**、直接断言模型读出了图里的 `600519`，把「图确实到达模型」钉死。**连带修掉一个使该守卫失效的测试隔离缺陷**：`image_stock_extractor` 在 import 期把 `sys.modules.get("litellm")` 绑成模块全局，而 23 个测试模块（外加被 83 个模块在 import 期调用的 `tests/litellm_stub.ensure_litellm_stub`）会在收集期把 MagicMock 塞进 `sys.modules["litellm"]`，于是这条守卫在全量跑里拿到的是 MagicMock、假红成「图不再被转发」——它本该守住的正是这件事。现所有注入点统一改为「先真 import，仅 `ModuleNotFoundError`/`ImportError` 才注入」，并在 `tests/conftest.py` 模块级 `os.environ.setdefault("LITELLM_MODE", "PROD")`：否则真实 litellm 在 import 期执行 `load_dotenv()` 会把开发机 `.env` 读进测试进程（实测不设该变量时 `import litellm` 后 `os.environ["LITELLM_FALLBACK_MODELS"]` 变成 `.env` 里的值，设为 `PROD` 后保持未设置），而 `.env` 里未声明的 fallback 会让 `test_system_config_service` / `test_system_config_api` / `test_provider_cache` 这一类测试假红——既有隔离缺陷，此前被 stub 掩盖
- [测试] 静态 API 规格与图片错误文案的守卫补齐：`tests/test_chat_image_vision_routing.py` 的路由映射用例（provider 专属路由改走 `openai/`、原生可用路由不被改写、幂等、deployment 内显式 model 优先）覆盖「图被转发」之外的另一半——路由没被改对时同样静默失效；`tests/test_chat_prompt_image_rule.py` 同时钉住「必须提案」与「不得仅因图中出现代码而提案」两半，并要求两套 chat 提示词的规则 10 **逐字相同**、Codex 提示词不得出现该规则（反向验证：删掉「明确表达」或把规则加进 Codex 提示词时对应用例转红）
