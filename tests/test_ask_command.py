...
class TestAskCommandActionProposalHint(unittest.TestCase):
    """Verify /ask surfaces every action proposal and sends users to the Web client."""
    ...
    def _reply_for_events(self, *events):
        """Run _analyze_single with an executor that replays the given progress events."""
        command = AskCommand()
    ...
    def test_every_kind_points_to_the_web_client(self):
        for kind, summary in (
            ...
            with self.subTest(kind=kind):
                response = self._reply_for_events(
                ...
                # 本仓库没有 /alert 命令（ALL_COMMANDS 里没有 alert）：提示用户执行它
                # 只会得到「未知命令: alert」，所以任何 kind 都不能出现这条指引。
                self.assertNotIn("/alert", response.text)
                # 提案不是预警：说成「可创建的预警」就是给用户错话。
                self.assertNotIn("预警", response.text)

    def test_every_proposal_in_one_session_is_listed(self):
        response = self._reply_for_events(
            {"type": "action_proposal", "kind": "watchlist_add", "summary": "把「600519」加入自选"},
            {"type": "action_proposal", "kind": "alert", "summary": "「600519」价格上穿 ¥1800"},
        )
        # 两次提案都要在回复里：只留最后一条会让先到的提案对 Feishu 用户完全不可见。
        self.assertIn("把「600519」加入自选", response.text)
        self.assertIn("「600519」价格上穿 ¥1800", response.text)
        self.assertEqual(response.text.count("（请在 Web 端「问股」中确认）"), 1)

    def test_malformed_and_other_events_add_no_hint(self):
        response = self._reply_for_events(
            {"type": "tool_done", "tool": "get_realtime_quote"},
            {"type": "action_proposal", "summary": "   "},
            {"type": "action_proposal", "summary": 42},
